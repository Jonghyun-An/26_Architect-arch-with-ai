...
export class HealthController {
  ...
  root(@Res() res: any, @Req() req: any) {
    ...
    const distIndexPath = resolve(__dirname, '..', 'src', 'ui', 'analysis', 'dist', 'index.html');
    const srcIndexPath = resolve(__dirname, '..', 'src', 'ui', 'analysis', 'index.html');
    ...
    // 빌드 산출물이 없으면 기본 응답
    return res.json({
      message: 'Count Analysis Service',
      status: 'running',
      ui: 'React UI is not built yet. Please run: cd src/ui/analysis && npm install && npm run build',
    });
  }
}
